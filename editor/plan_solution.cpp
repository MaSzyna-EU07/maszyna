...
module eu07.editor.plan_solution;
...
bool pose_at(const SolvedTrack& track, double station, geometry::Pose& out) {
    const auto& line = track.centreline;
    if (line.empty()) {
        return false;
    }
    if (line.size() == 1) {
        out = geometry::Pose{line[0].x, line[0].y, track.start.hx, track.start.hy};
        return true;
    }
    ...
    for (std::size_t i = 1; i < line.size(); ++i) {
        ...
        if (station <= walked + step) {
            ...
            out = geometry::Pose{line[i - 1].x + dx * t, line[i - 1].y + dy * t,
                                         dx / step, dy / step};
            return true;
        }
        walked += step;
