...
export module eu07.simcore:driver;
import :fwd;
import :dynobj;
import eu07.glm;
import :mover;
...
//----------------------------------------------------------------------------
inline const bool Aggressive = true;
inline const bool Easyman = false;
inline const bool AIdriver = true;
inline const bool Humandriver = false;
inline const int maxorders = 64; // ilość rozkazów w tabelce
inline const int maxdriverfails = 4; // ile błędów może zrobić AI zanim zmieni nastawienie
extern bool WriteLogFlag; // logowanie parametrów fizycznych
inline const int BrakeAccTableSize = 20;

inline const int gbh_NP = -2; //odciecie w hamulcu ogolnym
inline const int gbh_RP = 0; //jazda w hamulcu ogolnym
inline const int gbh_FS = -1; //napelnianie uderzeniowe w hamulcu ogolnym
inline const int gbh_MIN = -2; //minimalna pozycja w hamulcu ogolnym
inline const int gbh_MAX = 6; //maksymalna pozycja w hamulcu ogolnym
//----------------------------------------------------------------------------
...
// members
    ...
    int OrderTop = 0; // wolne miejsce do wstawiania nowych
...
public:
// Formerly private, reached through friendship by TTrain and the ui panels.
// Those live in other modules, and a friend declaration would make TController
// the first declarer of a class defined elsewhere, which C++20 modules reject.
// The class TODO above ("few authorized inspectors, or bunch of public getters?")
// is still open; until it is answered these are plain public members.
    ...
    int DrivigFlags() const { return iDrivigFlags; }
    ...
    int OrderPos = 0; // rozkaz aktualny
...
public:
// Read by the ui panels, which are no longer friends: a friend declaration
