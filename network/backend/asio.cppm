...
export module eu07.network.backend.asio;
...
// The implementation lives here rather than in a separate implementation unit:
// asio defines its customization points as objects in an anonymous namespace, and
// GCC emits those a second time in any implementation unit that also includes
// asio.hpp, which the assembler rejects as duplicate symbols. One translation unit
// including asio is the way around it.
