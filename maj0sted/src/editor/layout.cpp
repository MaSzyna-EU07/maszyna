...
domain::TurnoutPart part_of(int part) {
    switch (part) {
        case 0: return domain::TurnoutPart::PreBlade;
        case 1: return domain::TurnoutPart::Blade;
        case 3: return domain::TurnoutPart::FrogRail;
        default: return domain::TurnoutPart::Curve;
    }
}

int mark_kind_for(domain::TurnoutPart part) {
    switch (part) {
        case domain::TurnoutPart::PreBlade: return 0;  // PR
        case domain::TurnoutPart::Blade: return 1;     // ostrze iglicy
        case domain::TurnoutPart::Curve: return 2;     // pięta iglicy
        case domain::TurnoutPart::FrogRail: return 4;  // koniec krzywej
    }
    return 3;
}
...
domain::Turnout to_domain(const TurnoutType& type, int hand) {
    domain::Turnout turnout{
        domain::CrossingMark{type.crossing_n},
        hand >= 0 ? domain::DivergeSide::Left : domain::DivergeSide::Right,
        {},
        type.length,
        domain::Blade{type.blade.tip_thickness, type.blade.nose, type.blade.railtop_width},
    };
    turnout.path.reserve(type.pieces.size());
    for (const auto& piece : type.pieces) {
        turnout.path.push_back(domain::TurnoutPiece{part_of(piece.part), piece.length,
...
std::vector<TurnoutMark> turnout_marks(const domain::TurnoutGeometry& geometry,
    ...
    domain::geometry::Pose walker = start;
    double station = 0.0;
    domain::TurnoutPart previous = domain::TurnoutPart::PreBlade;
    bool first = true;
    for (const auto& segment : geometry.path) {
        ...
        walker = domain::geometry::layout_segment(segment.k0, segment.k1, segment.length,
                                                  walker, nullptr);
    ...
    for (const std::size_t index : order) {
        ...
        switch (node.kind) {
            case NodeKind::Turnout: {
                ...
                const domain::TurnoutGeometry geometry =
                    domain::lay_turnout(at, to_domain(*type, placement.hand), bend);
                if (!geometry.valid) {
                    Diagnostic diagnostic;
                ...
                for (const auto& segment : geometry.path) {
                    ...
                    walker = domain::geometry::layout_segment(segment.k0, segment.k1,
