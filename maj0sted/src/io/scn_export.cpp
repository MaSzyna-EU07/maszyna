...
using maj0sted::editor::Document;
...
using maj0sted::editor::SolvedTurnout;
using maj0sted::editor::TurnoutPlacement;
...
double catalogue_radius(const editor::TurnoutType& type) {
    double longest = 0.0;
    double radius = 0.0;
...
double heading_degrees(const domain::geometry::Pose& pose) {
...
ScnExportResult export_scn(const Document& document,
                           const maj0sted::editor::Solution& solution,
    ...
    for (const auto& track : solution.tracks) {
        ...
        const auto write_switch = [&](const Span& span) {
            ...
            domain::geometry::Pose exit_pose{};
