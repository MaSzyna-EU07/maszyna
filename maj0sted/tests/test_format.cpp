...
void round_trip_is_byte_identical() {
    ...
    const std::string text = maj0sted::io::serialize(document);

    const auto reloaded = maj0sted::io::deserialize(text);
    CHECK(reloaded.has_value());
    if (!reloaded) {
        return;
    }
    CHECK(same(document, *reloaded));
    CHECK(maj0sted::io::serialize(*reloaded) == text);
}

void every_field_survives() {
    const Document document = sample();
    const auto reloaded = maj0sted::io::deserialize(maj0sted::io::serialize(document));
    CHECK(reloaded.has_value());
    if (!reloaded) {
        return;
    }
...
void an_older_file_is_refused() {
    ...
    const std::string version_1 =
        "maj0sted 1\ncrs 2180\nniwelety 0\neditor 1\neniw 2 1\nename niweleta 1\n";
    CHECK(!maj0sted::io::deserialize(version_1).has_value());
...
void a_file_from_before_lukowanie_still_opens() {
    ...
    const std::string version_5 =
        "m0s 5\nnext 5\nview 0 0 0\norigin 0 0 0 0\ntypes 0\n"
        "turnouts 1\nturnout 2 1 100 1 1\ntutype Rz 1:9 R190\ntracks 0\n";
    const auto document = maj0sted::io::deserialize(version_5);
...
void a_file_from_before_the_nose_still_opens() {
    ...
    const std::string version_4 =
        ...
        "tname Rz 1:9 R190\nturnouts 0\ntracks 0\n";
    const auto document = maj0sted::io::deserialize(version_4);
    CHECK(document.has_value());
    if (!document) {
        return;
    }
...
void an_old_file_upgrades_to_the_piece_list() {
    ...
    const std::string version_3 =
        ...
        "turnouts 0\ntracks 0\n";
    const auto document = maj0sted::io::deserialize(version_3);
    CHECK(document.has_value());
    if (!document) {
        return;
    }
    ...
    const auto laid = maj0sted::domain::lay_turnout(
        maj0sted::domain::geometry::Pose{0.0, 0.0, 1.0, 0.0}, to_domain(type, 1));
    CHECK(laid.valid);
    CHECK(std::abs(laid.angle_residual) < 1e-9);
    CHECK(std::abs(laid.length_residual) < 1e-9);

    // and it is written back in the new form
    CHECK(maj0sted::io::serialize(*document).find("tseg ") != std::string::npos);
}

void a_truncated_file_is_refused() {
    const std::string text = maj0sted::io::serialize(sample());
    // Cut off half way: the counts no longer match what follows.
    CHECK(!maj0sted::io::deserialize(text.substr(0, text.size() / 2)).has_value());
}
